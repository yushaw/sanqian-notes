...
export class ObsidianImporter extends BaseImporter {
  ...
  async parse(options: ImportOptions): Promise<ParsedNote[]> {
    ...
    // 收集所有 Markdown 文件
    const files = this.collectMarkdownFiles(sourcePath)
  ...
  private async parseFile(
    ...
    const attachments = options.importAttachments
      ? this.collectObsidianAttachments(content, dirname(filePath), rootPath)
      : []
   ...
   * 收集 Obsidian 格式的附件引用
   */
  private collectObsidianAttachments(
    content: string,
    basePath: string,
    rootPath: string
  ): PendingAttachment[] {
    ...
    while ((match = wikiImageRegex.exec(content)) !== null) {
      ...
      // 尝试在 vault 中查找文件
      const absolutePath = this.findAttachmentInVault(fileName, basePath, rootPath)
    ...
    while ((match = mdImageRegex.exec(content)) !== null) {
      ...
      // 尝试在 vault 中查找文件
      const absolutePath = this.findAttachmentInVault(imagePath, basePath, rootPath)
      if (absolutePath && !seen.has(absolutePath)) {
        seen.add(absolutePath)
        attachments.push({
          originalRef: ref,
  ...
  private findAttachmentInVault(
    ...
    if (existsSync(relativePath) && this.isPathSafe(relativePath, rootPath)) {
      return relativePath
    }

    // 2. 尝试 vault 根目录
    const rootRelative = join(rootPath, fileName)
    if (existsSync(rootRelative) && this.isPathSafe(rootRelative, rootPath)) {
      return rootRelative
    }
    ...
    for (const dir of commonDirs) {
      ...
      if (existsSync(inCommonDir) && this.isPathSafe(inCommonDir, rootPath)) {
        return inCommonDir
      }
    }
   ...
   * 在目录中搜索文件（限制深度）
   */
  private searchFileInDir(dir: string, fileName: string, maxDepth: number): string | null {
    ...
    try {
      ...
      for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
          const found = this.searchFileInDir(fullPath, fileName, maxDepth - 1)
