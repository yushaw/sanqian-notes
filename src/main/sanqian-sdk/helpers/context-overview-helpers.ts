...
import type { ContextOverviewDataSource, ContextOverviewNote } from '../../context-overview'
import {
  getNotebooks,
  getNotesByUpdated,
...
export function resolveEditorNoteRefByTitle(input: { noteTitle: string }): { noteId: string; noteTitle: string } | null {
  ...
  const localCandidates = buildLocalContextListItems(notebookNameMap, query).map((item) => ({
    id: item.id,
